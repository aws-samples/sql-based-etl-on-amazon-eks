...
1. Check your connection in [AWS CloudShell](https://console.aws.amazon.com/cloudshell/) or local computer. If no access, run the [post-deployment script](#run-a-script).

```bash
kubectl get svc
```
2. Login to Argo workflow website. The authentication token refreshes every 10mins (configurable).

```bash
# get ARGO Website
echo -e "\nGo to Argo website: $(aws cloudformation describe-stacks --stack-name SparkOnEKS \
--query "Stacks[0].Outputs[?OutputKey=='ARGOURL'].OutputValue" --output text)\n"
# get login token
echo -e "\nCopy & paste the token to login:\n\n$(argo auth token)\n"
```
