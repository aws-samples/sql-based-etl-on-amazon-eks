...
2. Open a sample job `spark-on-eks/source/example/notebook/scd2-job.ipynb` from the Jupyter notebook instance. Click `refresh` button if can't see the file. The job outputs a table to support the [Slowly Changing Dimension Type 2](https://www.datawarehouse4u.info/SCD-Slowly-Changing-Dimensions.html) business need.
